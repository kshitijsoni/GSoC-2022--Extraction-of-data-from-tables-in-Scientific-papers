...
path_f = "C:\TabularData_Extractor\PROCESSED FILES 2"
path_out = "C:\TabularData_Extractor\OUTPUT FILES"
#path_f_1 = r"C:\TabularData_Extractor\PROCESSED FILES 2"
folders = os.listdir(path_f)
for i in range(0, len(folders)):
#    path_i = "C:\TabularData_Extractor\PROCESSED FILES" 
    folder_i = folders[i]
    row_print = 0
    #path_out_i = path_out + '\output_' + str(i + 1) + '.xlsx'
    path_out_i = "C:\TabularData_Extractor\OUTPUT FILES\{}.xlsx".format(file_name[i])
    workbook = xlsxwriter.Workbook(path_out_i)
    worksheet = workbook.add_worksheet() 
    path_i = "C:\TabularData_Extractor\PROCESSED FILES 2\{}".format(i + 1)
    folders_i = os.listdir(path_i)
    for p in range(0, len(folders_i)):
        folder_p = folders_i[p]
        if folder_p == 'Detected Tables':
            path_u = "C:\TabularData_Extractor\PROCESSED FILES 2\{}\Detected Tables".format(i + 1) 
            folders_i = os.listdir(path_u)
            for j in range(0, len(folders_i)):
                index=int(folders_i[j])
                path_j = "C:\TabularData_Extractor\PROCESSED FILES 2\{}\Detected Tables\{}".format(i + 1,index)
                folders_j = os.listdir(path_j)
                for k in range(0, len(folders_j)):
                    tables_1 = cv2.imread(r"C:\TabularData_Extractor\PROCESSED FILES 2\{}\Detected Tables\{}\{}\image.jpg".format(i + 1 , index , k + 1) ,0)
                    cv2.imwrite("adstable.jpg" , tables_1)
                    tables_vertical = cv2.imread(r"C:\TabularData_Extractor\PROCESSED FILES 2\{}\Detected Tables\{}\{}\vertical.jpg".format(i + 1 , index , k + 1) ,0)
                    cv2.imwrite("adsver.jpg" , tables_vertical)
                    tables_horizontal = cv2.imread(r"C:\TabularData_Extractor\PROCESSED FILES 2\{}\Detected Tables\{}\{}\horizontal.jpg".format(i + 1 ,index , k + 1) ,0)
                    cv2.imwrite("adshor.jpg" , tables_horizontal)
                    row_print = main_table_1(tables_1, row_print, tables_vertical, tables_horizontal)
                    
        
        else:
            tables_1 = cv2.imread(r"C:\TabularData_Extractor\PROCESSED FILES 2\{}\{}\image.jpg".format(i + 1 ,p + 1)  ,0)
            cv2.imwrite("ewewew.jpg", tables_1)
            tables_vertical = cv2.imread(r"C:\TabularData_Extractor\PROCESSED FILES 2\{}\{}\vertical.jpg".format(i + 1 ,p + 1)  ,0)
            cv2.imwrite("ewewver.jpg", tables_vertical)
            tables_horizontal = cv2.imread(r"C:\TabularData_Extractor\PROCESSED FILES 2\{}\{}\horizontal.jpg".format(i + 1,p + 1)  ,0)
